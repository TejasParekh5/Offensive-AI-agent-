...
class FlaskReportGenerator:
    ...
    def generate_pdf_report(self, assessment_data: Dict[str, Any], 
        ...
        """
        if not REPORTLAB_AVAILABLE:
            raise RuntimeError("ReportLab not available - cannot generate PDF reports")
        
        # Create output path if not provided
        if not output_path:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            assessment_id = assessment_data.get('assessment', {}).get('id', 'unknown')
        ...
        try:
            ...
            story.extend(self._generate_pdf_cover_page(assessment_data, title_style, styles))
            story.append(PageBreak())
            
            story.extend(self._generate_pdf_executive_summary(assessment_data, heading_style, styles))
            story.append(PageBreak())
            
            story.extend(self._generate_pdf_methodology(assessment_data, heading_style, styles))
            story.append(PageBreak())
            
            story.extend(self._generate_pdf_findings(assessment_data, heading_style, styles))
            story.append(PageBreak())
            
            story.extend(self._generate_pdf_recommendations(assessment_data, heading_style, styles))
            story.append(PageBreak())
            
            story.extend(self._generate_pdf_appendix(assessment_data, heading_style, styles))
    ...
    def _generate_pdf_cover_page(self, assessment_data: Dict[str, Any], title_style, styles) -> List:
        ...
        assessment_details = [
            ...
            ['Start Date:', assessment.get('created_at', 'N/A')[:10] if assessment.get('created_at') else 'N/A'],
            ['Completion Date:', assessment.get('completed_at', 'N/A')[:10] if assessment.get('completed_at') else 'N/A'],
        ...
        risk_data = [
            ...
            ['Critical Issues:', str(summary.get('severity_breakdown', {}).get('critical', 0))],
            ['High Issues:', str(summary.get('severity_breakdown', {}).get('high', 0))]
        ]
    ...
    def _generate_pdf_findings(self, assessment_data: Dict[str, Any], heading_style, styles) -> List:
        ...
        if recon_results:
            story.append(Paragraph("Reconnaissance Results", styles['Heading3']))
        ...
        if scan_results:
            story.append(Paragraph("Port Scanning Results", styles['Heading3']))
        ...
        if test_results:
            story.append(Paragraph("Security Test Results", styles['Heading3']))
            ...
            for severity in ['critical', 'high', 'medium', 'low']:
                ...
                if tests:
                    story.append(Paragraph(f"{severity.capitalize()} Severity Issues", styles['Heading4']))
    ...
    def _generate_pdf_recommendations(self, assessment_data: Dict[str, Any], heading_style, styles) -> List:
        ...
        if severity_breakdown.get('critical', 0) > 0:
            recommendations.append("• Address all critical severity vulnerabilities immediately")
            recommendations.append("• Implement emergency security controls and monitoring")
        ...
        if severity_breakdown.get('medium', 0) > 0:
            recommendations.append("• Schedule remediation of medium severity vulnerabilities")
            recommendations.append("• Enhance security monitoring and detection capabilities")
        ...
        # Priority matrix
        story.append(Paragraph("Remediation Priority Matrix", styles['Heading3']))
    ...
    def _generate_pdf_appendix(self, assessment_data: Dict[str, Any], heading_style, styles) -> List:
        ...
        test_results = assessment_data.get('test_results', [])
        if test_results:
            story.append(Paragraph("Test Execution Summary", styles['Heading4']))
            
            test_summary_data = [['Test Name', 'Category', 'Severity', 'Status', 'Vulnerable']]
            ...
            test_table = Table(test_summary_data, colWidths=[2*inch, 1*inch, 1*inch, 1*inch, 0.8*inch])
