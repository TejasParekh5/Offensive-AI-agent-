...
class FlaskTestAgent:
        ...
        try:
            if callback:
                callback(generation_id, {
                         'status': 'generating', 'message': 'Analyzing target information'})
            ...
            if callback:
                callback(generation_id, {
                         'status': 'generating', 'message': 'Generating test cases with LLM'})
            ...
            if callback:
                callback(generation_id, {
                         'status': 'generating', 'message': 'Processing generated test cases'})

            self.test_status[generation_id]['progress'] = 80

            # Process and validate generated test cases
            processed_tests = self._process_generated_tests(
                generated_tests, target_info)
            ...
            self.logger.error(
                f"LLM test generation {generation_id} failed: {e}")
            self.test_results[generation_id] = {
                'generation_id': generation_id,
                'status': 'failed',
                'error': str(e),
    ...
    def _prepare_llm_context(self, target_info: Dict[str, Any], scan_results: Dict[str, Any] = None) -> str:
        ...
        context_parts.append(
            f"Target Type: {target_info.get('type', 'unknown')}")
            ...
            if 'hosts' in scan_results:
                for host in scan_results['hosts']:
                    if 'ports' in host:
                        ...
                        if open_ports:
                            context_parts.append(
                                f"Host {host.get('ip', 'unknown')} has open ports:")
        ...
        if 'recon_results' in target_info:
            ...
            if 'technologies' in recon:
                context_parts.append(
                    f"Technologies detected: {', '.join(recon['technologies'])}")

            if 'emails' in recon:
                context_parts.append(
                    f"Email addresses found: {len(recon['emails'])}")
        ...
        for i, test in enumerate(generated_tests):
            try:
                ...
                if not all(key in test for key in ['name', 'description', 'category']):
                    self.logger.warning(
                        f"Generated test {i} missing required fields, skipping")
                    continue
                ...
                self.logger.warning(
                    f"Failed to process generated test {i}: {e}")
                continue
        ...
        try:
            ...
            for i, test_case in enumerate(test_suite):
                ...
                test_result = self._execute_single_test(
                    test_case, target, execution_mode)
                self.test_results[execution_id]['test_results'].append(
                    test_result)
            ...
            self.test_results[execution_id]['end_time'] = datetime.now(
            ).isoformat()
    ...
    def _execute_single_test(self, test_case: Dict[str, Any], target: str,
        ...
        try:
            ...
            self.logger.error(
                f"Test execution failed for {test_case.get('name', 'unknown')}: {e}")
            test_result.update({
                'status': 'error',
                'error': str(e),
                'end_time': datetime.now().isoformat()
    ...
    def _execute_payload_test(self, test_case: Dict[str, Any], target: str) -> Dict[str, Any]:
        ...
        vulnerable_count = sum(
            1 for r in results if r.get('vulnerable', False))
    ...
    def _execute_credential_test(self, test_case: Dict[str, Any], target: str) -> Dict[str, Any]:
        ...
        # Determine overall status
        successful_auths = sum(1 for r in results if r.get(
    ...
    def approve_llm_test(self, generation_id: str, test_id: str) -> bool:
        ...
        if generation_id in self.test_results:
            generated_tests = self.test_results[generation_id].get(
                'generated_tests', [])
