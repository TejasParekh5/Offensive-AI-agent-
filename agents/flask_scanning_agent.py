...
class FlaskScanningAgent:
    ...
    def __init__(self):
        ...
        self.logger.info(
            f"FlaskScanningAgent initialized. Available tools: {[tool for tool, config in self.scan_tools.items() if config['available']]}")
    ...
    def _run_scan(self, scan_id: str, target: str, scan_type: str, ports: str, tool: str, callback=None):
        ...
        try:
            ...
            if callback:
                callback(scan_id, {'status': 'scanning',
                         'message': f'Starting {tool} scan of {target}'})

            # Execute scan based on tool
            if tool == 'nmap':
                results = self._run_nmap_scan(
                    scan_id, target, scan_type, ports, callback)
            elif tool == 'masscan':
                results = self._run_masscan_scan(
                    scan_id, target, scan_type, ports, callback)
            elif tool == 'rustscan':
                results = self._run_rustscan_scan(
                    scan_id, target, scan_type, ports, callback)
            else:
        ...
        while process.poll() is None:
            if callback:
                ...
                current_progress = min(
                    self.scan_status[scan_id]['progress'] + 10, 90)
