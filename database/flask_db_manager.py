...
class FlaskDatabaseManager:
    ...
    def _define_models(self):
        ...
        class Assessment(self.db.Model):
            ...
            id = self.db.Column(self.db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
            target = self.db.Column(self.db.String(255), nullable=False)
            target_type = self.db.Column(self.db.String(50), nullable=False)  # 'domain' or 'ip'
            status = self.db.Column(self.db.String(50), nullable=False, default='created')
            created_at = self.db.Column(self.db.DateTime, nullable=False, default=datetime.utcnow)
            updated_at = self.db.Column(self.db.DateTime, nullable=False, default=datetime.utcnow, onupdate=datetime.utcnow)
            completed_at = self.db.Column(self.db.DateTime, nullable=True)
            
            # Configuration
            scan_type = self.db.Column(self.db.String(50), default='comprehensive')
            ...
            # Relationships
            recon_results = self.db.relationship('ReconResult', backref='assessment', lazy=True, cascade='all, delete-orphan')
            scan_results = self.db.relationship('ScanResult', backref='assessment', lazy=True, cascade='all, delete-orphan')
            test_results = self.db.relationship('TestResult', backref='assessment', lazy=True, cascade='all, delete-orphan')
        ...
        class ReconResult(self.db.Model):
            ...
            id = self.db.Column(self.db.String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
            assessment_id = self.db.Column(self.db.String(36), self.db.ForeignKey('assessments.id'), nullable=False)
            
            # Timing
            started_at = self.db.Column(self.db.DateTime, nullable=False, default=datetime.utcnow)
            completed_at = self.db.Column(self.db.DateTime, nullable=True)
            
            # Status
            status = self.db.Column(self.db.String(50), nullable=False, default='running')
            error_message = self.db.Column(self.db.Text, nullable=True)
            
            # Results (stored as JSON)
            subdomains = self.db.Column(self.db.Text, nullable=True)  # JSON array
            dns_records = self.db.Column(self.db.Text, nullable=True)  # JSON object
            whois_data = self.db.Column(self.db.Text, nullable=True)  # JSON object
            technologies = self.db.Column(self.db.Text, nullable=True)  # JSON array
            emails = self.db.Column(self.db.Text, nullable=True)  # JSON array
            social_media = self.db.Column(self.db.Text, nullable=True)  # JSON array
            shodan_data = self.db.Column(self.db.Text, nullable=True)  # JSON object
        ...
        class ScanResult(self.db.Model):
            ...
            # Results (stored as JSON)
            hosts_data = self.db.Column(self.db.Text, nullable=True)  # JSON object
            summary_data = self.db.Column(self.db.Text, nullable=True)  # JSON object
        ...
        class TestResult(self.db.Model):
            ...
            test_source = self.db.Column(self.db.String(50), nullable=False)  # 'predefined' or 'llm_generated'
            ...
            # Status and results
            status = self.db.Column(self.db.String(50), nullable=False)  # 'passed', 'failed', 'error', 'skipped'
            vulnerable = self.db.Column(self.db.Boolean, nullable=False, default=False)
            error_message = self.db.Column(self.db.Text, nullable=True)
            
            # Test details (stored as JSON)
            test_data = self.db.Column(self.db.Text, nullable=True)  # Original test case
            result_data = self.db.Column(self.db.Text, nullable=True)  # Detailed results
    ...
    def create_assessment(self, target: str, target_type: str, scan_type: str = 'comprehensive',
        ...
        try:
            ...
            self.logger.info(f"Created assessment {assessment.id} for target {target}")
    ...
    def save_recon_results(self, assessment_id: str, results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.info(f"Saved recon results for assessment {assessment_id}")
    ...
    def save_scan_results(self, assessment_id: str, results: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.info(f"Saved scan results for assessment {assessment_id}")
        ...
        try:
            ...
            for test_result in test_results:
                test_record = self.TestResult(
                    ...
                    completed_at=datetime.utcnow() if test_result.get('status') != 'running' else None,
            ...
            self.logger.info(f"Saved {len(test_results)} test results for assessment {assessment_id}")
            return result_ids
    ...
    def get_assessment_results(self, assessment_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            total_vulnerabilities = sum(1 for t in test_results if t.get('vulnerable', False))
