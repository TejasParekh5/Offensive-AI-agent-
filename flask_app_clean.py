...
app.config['SECRET_KEY'] = os.environ.get(
    'SECRET_KEY', 'cybersec-automation-key-2025')
...
def dashboard():
    ...
    try:
        ...
        return render_template('dashboard.html',
                               recent_assessments=[],
                               stats={'total_assessments': 0, 'active_assessments': 0,
                                      'total_vulnerabilities': 0, 'completed_today': 0},
                               llm_status=False)
...
def download_report(assessment_id, format):
    ...
    try:
        ...
        if format == 'pdf':
            ...
            report_path = report_generator.generate_csv_summary(
                assessment_data)
        else:
...
if HAS_SOCKETIO and socketio:
    ...
    def handle_join_assessment(data):
        ...
        if assessment_id and join_room:
            ...
            logger.info(
                f"Client {request.sid} joined assessment {assessment_id}")
...
def _run_assessment_async(assessment_id, config):
    ...
    try:
        logger.info(
            f"Starting assessment {assessment_id} for target {config['target']}")
