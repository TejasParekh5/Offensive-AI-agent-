        ...
        if include_llama:
            ...
            for dep in llama_deps:
                try:
                    ...
                    colored_print(f"✅ {dep.split('>=')[0]} installed", Colors.GREEN)
...
def check_system_requirements():
    ...
    try:
        ...
        if memory_gb >= 16:
            colored_print(f"✅ Memory: {memory_gb:.1f}GB (Excellent for Llama)", Colors.GREEN)
        elif memory_gb >= 8:
            colored_print(f"✅ Memory: {memory_gb:.1f}GB (Good with quantization)", Colors.YELLOW)
        else:
            colored_print(f"⚠️ Memory: {memory_gb:.1f}GB (Limited, may affect performance)", Colors.YELLOW)
    except ImportError:
        colored_print("⚠️ Cannot check memory (psutil not installed)", Colors.YELLOW)
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            colored_print(f"✅ CUDA: {gpu_count} GPU(s), {gpu_memory:.1f}GB VRAM", Colors.GREEN)
        else:
            colored_print("⚠️ CUDA not available - CPU will be used", Colors.YELLOW)
    except ImportError:
    ...
    if free_gb >= 25:
        ...
        colored_print(f"⚠️ Disk space: {free_gb:.1f}GB free (may be insufficient for Llama)", Colors.YELLOW)
...
def test_imports():
    ...
    try:
        ...
        import streamlit, pandas, plotly, requests, jinja2
...
def test_database():
    ...
    try:
        ...
        session_created = db.create_session("test_session", "test_target", "domain")
        if session_created:
...
def setup_llama():
    ...
    try:
        ...
        colored_print("⬇️ Downloading Llama model (this may take time)...", Colors.CYAN)
        ...
        if "gated repo" in str(e).lower() or "access" in str(e).lower():
            ...
            colored_print("1. Visit: https://huggingface.co/meta-llama/Meta-Llama-3.1-8B-Instruct", Colors.CYAN)
            colored_print("2. Request access and login: huggingface-cli login", Colors.CYAN)
...
def test_llama():
    ...
    try:
        ...
        if llama.is_available():
            ...
            if test_cases:
                colored_print(f"✅ Generated {len(test_cases)} test cases", Colors.GREEN)
            else:
...
def display_help():
    """Display usage help."""
    colored_print("🛡️ Multi-Agent Cybersecurity Automation - Setup & Test Utility", Colors.BOLD)
    print("""
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Cybersecurity System Setup & Test")
    ...
    colored_print("🛡️ Multi-Agent Cybersecurity Automation System", Colors.BOLD)
    ...
    if success:
        ...
        colored_print("2. Launch dashboard: python main.py dashboard", Colors.WHITE)
        colored_print("3. Or run CLI: python main.py scan --target example.com", Colors.WHITE)
    else:
        colored_print("⚠️ Some operations failed. Check the output above.", Colors.YELLOW)
